...
export default function CategoryGrid({
  ...
  return (
    <div className="mb-10">
      <h2 className="mb-3 text-lg font-semibold text-neutral-900">Kategoriler</h2>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
        {categories.map((c) => (
          <Link
            key={c.id}
            href={`/?kategori=${c.id}`}
            className="group rounded-xl border border-neutral-200 p-4 transition-colors hover:border-[var(--color-brand)]"
          >
